...
public class UserAppService : PaperManagementAppService, IUserAppService
{
    ...
    {
        var user = await _keycloakUserClient.GetUserAsync(_keycloakOptions.Value.Realm,id.ToString());
        return ObjectMapper.Map<UserRepresentation, UserDto>(user);
    }

    public async Task<PagedResultDto<UserDto>> GetListAsync(PagedSortedAndFilteredResultRequestDto input)
    {
        var queryable = (await _keycloakUserClient.GetUsersAsync(_keycloakOptions.Value.Realm)).AsQueryable();
        ...
        if (!(input.FilterField.IsNullOrEmpty() || input.FilterValue.IsNullOrEmpty()))
        {
            ...
            {
                nameof(UserDto.Username) => queryable.Where(t => t.Username!.Contains(input.FilterValue!)),
                _ => queryable
            };
        }
        ...
        return new PagedResultDto<UserDto>(total, ObjectMapper.Map<List<UserRepresentation>, List<UserDto>>(list));
    }
}